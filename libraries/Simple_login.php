 ...
 class Simple_login {
     ...
     */
     public function login($username, $password) {
         ...
         $query = $this->CI->db->get_where('fab_member',array('username'=>$username,'password' => md5($password), 'status'=>'1'));
 
         if($query->num_rows() == 1) {
             //ambil data user berdasar username
             $row       = $this->CI->db->query('SELECT id,username,level,nama FROM fab_member where username = "'.$username.'"');
             $admin     = $row->row();
             $id        = $admin->id;
             $level     = $admin->level;
             $userid    = $admin->userid;
